angular.module('aws') .controller('loginController', [
						...
						function($scope, $http, $location, $localStorage,
								$timeout,aws_config,admin_aws_config,admin_cognito_config,restAPI_url,cognito_config, $rootScope) {
							var userPool;
							$scope.langchecked = 0;
							...
							$scope.init = function() {
								...
								var adminPool = new AWSCognito.CognitoIdentityServiceProvider.CognitoUserPool(admin_cognito_config.poolData);
								var adminsession = adminPool.getCurrentUser();*/
								...
								var usersession = userPool.getCurrentUser();
							...
							$scope.setAdminPool = function(username, password) {
								...
								console.log('admin_aws_config:',admin_aws_config);
								admin_cognito_config.poolData.UserPoolId = admin_aws_config.userPoolId,
								admin_cognito_config.poolData.ClientId = admin_aws_config.appClientId
								admin_cognito_config.userPool = new AWSCognito.CognitoIdentityServiceProvider.CognitoUserPool(admin_aws_config.poolData);
								admin_cognito_config.userData.Username = username;
								admin_cognito_config.userData.Pool = admin_aws_config.adminPool;
								admin_cognito_config.cognitoUser = new AWSCognito.CognitoIdentityServiceProvider.CognitoUser(admin_aws_config.userData);
									var authenticationData = {
										Username : username,
										Password : password
									};
									admin_aws_config.authenticationDetails = new AWSCognito.CognitoIdentityServiceProvider.AuthenticationDetails(authenticationData);
								...
								admin_cognito_config.cognitoUser.authenticateUser(admin_cognito_config.authenticationDetails,
												{  									
													onSuccess : function(result) {
														...
														$rootScope.tokenId = result.getIdToken().getJwtToken();  	
														//console.log("id token:"+$rootScope.tokenId);
														admin_cognito_config.idpKey = ('cognito-idp.'+ admin_cognito_config.region + '.amazonaws.com/' + admin_cognito_config.userPoolId);
														var key = admin_cognito_config.idpKey;
														AWS.config.credentials = new AWS.CognitoIdentityCredentials({
														IdentityPoolId : admin_cognito_config.identityPoolId, 
															Logins : {
         																key : result.getIdToken().getJwtToken()
																	}
																});
														
														cognito_config.cognitoUser.getUserAttributes(function(err,result) {
																	if (err) {
																	}
							...
							$scope.validateUser = function() {
								...
								if($rootScope.userRole == "user"){
									...
									alert("selected role::"+$scope.role)
